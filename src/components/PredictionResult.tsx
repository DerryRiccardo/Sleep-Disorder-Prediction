	...
	return (
		<div
			...
			<Card className={`border-2 ${bgColorMap[result.prediction]}`}>
				<CardHeader>
					<div className="flex items-center gap-4">
						<div
							className={`p-3 rounded-full ${bgColorMap[result.prediction]}`}>
					...
					<ul className="space-y-3">
						{recommendation.tips.map((tip, index) => (
							<li
								...
								<span className="text-foreground dark:text-white">{tip}</span>
							</li>
						))}
					</ul>
				</CardContent>
			...
			<p className="text-xs text-muted-foreground text-center px-4">
				...
				Selalu konsultasikan dengan profesional kesehatan untuk evaluasi yang
				akurat.
			</p>
		</div>
	);
